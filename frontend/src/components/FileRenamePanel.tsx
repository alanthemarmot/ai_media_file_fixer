    ...
    <div className="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-lg font-semibold text-gray-900 dark:text-white">
          Rename File for {getContextDescription()}
        </h4>
        <button
          onClick={onCancel}
      ...
      <div className="space-y-3">
        <div>
          ...
          {selectedFile.detectedPath && (
            <div className="text-xs text-green-600 dark:text-green-400 mt-1">
              ✓ File path detected: {selectedFile.detectedPath}
            </div>
          )}
          {!selectedFile.detectedPath && (
        ...
        <div className="flex items-center space-x-3 pt-2">
          ...
          >
            ...
            <span>{isRenaming ? 'Renaming...' : 'Update File Name'}</span>
          </button>
        ...
        <div className="text-xs text-gray-500 dark:text-gray-400 mt-2">
          <strong>Note:</strong> This will rename the actual file on your file system.
        </div>
      </div>
    </div>
  );
